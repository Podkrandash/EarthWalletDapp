...
const connectUrl = async (page: Page, clipboardContent: string) => {
    if (process.env.BASE_APP_URL === 'https://earth-wallet.com') {
        await page.goto(clipboardContent);
        await page.getByRole('link', { name: 'Sign in with Earth Wallet Web' }).click();
    } else {
...
test.describe('ton connect', () => {
    test('elements', async ({ page, context }) => {
        ...
        await page1.getByRole('button', { name: 'Earth Wallet Popular' }).click();
