...
test('Full flow elements', async ({ page }) => {
  ...
  await expect(page.locator('form')).toContainText('Name your walletName your wallet to easily identify it while using the Earth Wallet. These names are stored locally, and can only be seen by you.');
  await expect(page.locator('form')).toContainText('Save');
  await page.getByRole('button', { name: 'Save' }).click();
  await expect(page.getByRole('heading', { name: 'Congratulations! You\'ve set up your wallet' })).toBeVisible();
