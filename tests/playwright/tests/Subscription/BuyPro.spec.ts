...
test('Buy Pro', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Continue' }).click();
  await
    page.locator('#react-portal-modal-container')
      .getByRole('textbox')
  ...
  await expect(page.getByText('Earth Wallet Pro', { exact: true })).toBeVisible();
  await expect(page.getByText('A Earth Wallet Pro subscription')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('A Earth Wallet Pro subscription includes advanced wallet functionality, offering a suite of tools for managing your cryptocurrency.');
  ...
  await expect(page.getByText('Inside Earth Wallet Pro –')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('Inside Earth Wallet Pro – Dashboard');
  await expect(page.locator('#react-portal-modal-container')).toContainText('In the Dashboard you will have access to wallet analysis. This feature will benefit professional users who use Earth Wallet in their daily business.');
  ...
  await expect(page.getByText('Multi-Send')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('With multi-send feature you can send funds up to 255 addresses at once. Earth Wallet Pro saves time and simplifies the process of mass transactions.');
  ...
  await expect(page.getByText('Earth Wallet Pro', { exact: true })).toBeVisible();
  await expect(page.getByText('Earth Wallet Pro\'s')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('Earth Wallet Pro\'s subscription comes with an extended wallet feature, offering a toolset for crypto management.');
  ...
  await expect(page.getByRole('heading', { name: 'Earth Wallet Pro' })).toBeVisible();
  await expect(page.getByText('Earth Wallet Pro\'s subscription')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('Earth Wallet Pro\'s subscription comes with an extended wallet feature, offering a toolset for crypto management.');
  ...
  await expect(page.locator('#react-portal-modal-container')).toContainText('Inside Earth Wallet Pro – Dashboard');
  await expect(page.locator('#react-portal-modal-container')).toContainText('In the Dashboard you will have access to wallet analysis. This feature will benefit professional users who use Earth Wallet in their daily business.');
  await expect(page.getByRole('button', { name: 'Get Pro' }).click());
  await expect(page.getByText('Earth Wallet Pro', { exact: true })).toBeVisible();
  await expect(page.getByText('Earth Wallet Pro\'s')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('Earth Wallet Pro\'s subscription comes with an extended wallet feature, offering a toolset for crypto management.');
  await expect(page.getByRole('button', { name: 'Continue with Pro' }).click());
  await expect(page.getByRole('heading', { name: 'Earth Wallet Pro' })).toBeVisible();
  await expect(page.getByText('Earth Wallet Pro\'s subscription')).toBeVisible();
  await expect(page.locator('#react-portal-modal-container')).toContainText('Earth Wallet Pro\'s subscription comes with an extended wallet feature, offering a toolset for crypto management.');
});